...
<form class="well form-search" method="post" action="{$gvar.l_admin_control_documentation}?option=save" enctype="multipart/form-data">
<legend>Edit documentation <span style="font-size:14px">(Fields marked with * are required)</span></legend>
<fieldset>
<table cellspacing="2"><tr><td align="right">Task:*</td><td>{$attr=$obj_aux->metadata()}<input id="{$attr['task']['foreign']}_{$attr['task']['foreign_attribute']}_{$attr['task']['foreign_name']}" value="{$object->get('task')}" name="task" required="required" type="text"> <button class="btn" onClick="window.open('{$gvar.l_admin_control}results_foreign.php?option={$attr['task']['foreign']}[*INTER*]{$attr['task']['foreign_name']}','popuppage','width=600,toolbar=1,resizable=1,scrollbars=yes,height=500,top=100,left=100');return false;"><img src="{$gvar.l_global}images/admin/key.png" alt="Assign Foreign" /></button></td>
...
</tr><tr><td align="right">Video:</td><td><input name="video" value="{$object->get('video')}" type="text"></td>
<td align="right">Link:</td><td><input name="link" value="{$object->get('link')}" type="text"></td>
</tr>
...
<tr>
<td align="right">Note:</td><td><textarea name="note">{$object->get('note')|escape}</textarea></td>
</tr>
<tr><td>
